...
public class Font
{
    ...
    public Font(string filename, int spacing, int line_height, int space_width)
    {
        ...
        if (ms == null)
        {
            Console.Error.WriteLine("Couldn't load \"{0}\" as data file for \"{1}\"", fnn + ".dat", filename);
            return;
        }
        ...
        for (int i = 0; i < wrapped.Length; i++)
        {
            if (wrapped[i].Length > 0)
            {
                ...
                for (int j = 0; j < wrapped[i].Length; j++)
                {
                    ...
                    qc[ppc++] = new Rgba32(1, 1, 1, 1);
                    qc[ppc++] = new Rgba32(1, 1, 1, 1);
                    qc[ppc++] = new Rgba32(1, 1, 1, 1);
                    qc[ppc++] = new Rgba32(1, 1, 1, 1);

                    RectangleF rec = CombinedTexture.AtlasRects[is_invisible ? 0 : c];
