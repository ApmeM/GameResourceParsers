...
public class MisToTmxConverter : BaseFileConverter<BinaryFile>
{
    protected override IEnumerable<BaseFile> ConvertFile(BinaryFile toConvert, List<BaseFile> files)
    {
        if (!toConvert.relativeFileExtension.Contains("MIS", StringComparison.InvariantCultureIgnoreCase))
        {
        ...
        map.TileSets = files.OfType<SpriteFile>().Select(a =>
            new TmxTileSet
            {
                Name = a.relativeFileName,
                TileWidth = a.Sprites[0].Width,
                TileHeight = a.Sprites[0].Height,
